...
wMa_pprel_filename = "/path_to/wMa.pprel.txt"
wMa_tf_filename = "/path_to/wMa.tf.txt"
...
parser.add_argument("-c", "--nTrain", type = int, nargs = 1,
                    ...
                    help = "Num of cells for training, '-1' for auto detect (2/3 of total cell number, may take time to count)") #this greatly effects runtime
parser.add_argument("-b", "--batchSize", type = int, nargs = 1,
                    metavar = "Num_cell_in_one_batch", default = 1,
                    help = "The median of the errors of the cells in one batch will be used for back propagation")
...
out_log.writelines("The parameters are:\nn_rounds: "+str(n_rounds)+" learning_rate: "+str(learning_rate)+" n_train: "+str(n_train)+" n_batch: "+str(n_batch)+".\n")
print("The parameters are:\nn_rounds:",n_rounds,"learning_rate:",learning_rate,"n_train:",n_train,"n_batch:",n_batch)
...
#load weight matrix
wMa_pprel_file = open (wMa_pprel_filename,'r')
...
wMa_tf_file = open (wMa_tf_filename,'r')
...
#n_train = 1000 #calculation of MSE is much faster than training the network.
...
for j in range(n_rounds): #for each round, the file will be opened once.
    ...
    inputs = numpy.array([])
    start = time.time()
    #while (sc_data_line): #start from the training cells. Have to follow the order of the cells in the input data file
    while (n_cells < n_train):
        n_cells = n_cells+1
        sc_data_line = sc_data_file.readline() #now this should be the cell id with gene expressions
        if(sc_data_line != ""):
            line = re.sub("[\n\r]","",sc_data_line).split(',') #remove the "\n\r" and split by ","
            cell_id = line[0]
            line = line[1:] #keep only the expression values. geneID removed the first element too.
            if(n_batch > 1):
                if(len(inputs) == 0):
                    inputs = numpy.array(line)[indx_input]
                    targets = numpy.array(line)[indx_target]
                else:
                    inputs = numpy.vstack((inputs,numpy.array(line)[indx_input]))
                    targets = numpy.vstack((targets,numpy.array(line)[indx_target]))
                    if(n_cells % n_batch == 0):
                        #print(inputs.shape,targets.shape,type(targets))
                        ...
                        for i in range(n_batch): #scale the inputs by its own max expr values
                        ...
                        inputs = numpy.array([])
                        targets = numpy.array([])
            #There might be some cells left after the last loop since n_train % n_batch may not be zero. It's risky to use these cells as one last batch to train the network.
            #The reason is that when the batch size is small, it's very possible that the non-zero output-errors of one gene among the cells in the batch come from very few genes,
            #or even just one gene (reason: for different cells, the original counts of this gene may be mostly zeros, but with even just one cell of positive counts, it'll be
            #take into account in the output_error calculation). Then the median or mean will under-estimate or over-estimate the contribution of this gene to the optimizer
            elif(n_batch == 1):
                inputs = numpy.array(line)[indx_input] #input genes, including zeros. Less than 3500
                targets = numpy.array(line)[indx_target] #!IMPORTANT: since indx_target is for all cells, there are genes whose expr are zeros in this array
    ...
    out_log.writelines("Training round "+str(j+1)+" takes "+str(end-start)+"s.\n")
    print("Training round ",j+1," takes ", end-start, "s.")

    out_log.writelines("Calculating error estimations for round "+str(j+1)+".\n")
    print("Calculating error estimations for round",j+1,".") #j is the number of rounds
    start = time.time()
    n_test = 0
    while sc_data_line: #move to the testing cells
        n_test = n_test +1
        if(n_test > 100000000): #Modify this numbr if want to control of the number of testing cells. Usually not necessary
            break
        sc_data_line = sc_data_file.readline()
        if(sc_data_line != ""):
            #begin to calculate errors for each cell
            line = re.sub("[\n\r]","",sc_data_line).split(',') #remove the "\n\r" and split by ","
            cell_id = line[0]
            #print(cell_id)
            line = line[1:] #keep only the expression values
            inputs = numpy.array(line)[indx_input] #input genes, including zeros. Less than 3500
            inputs = numpy.asfarray(inputs)
            inputs = numpy.log2(inputs +1) #log2 transformation
            targets = numpy.asfarray(line)[indx_target]
    ...
    if(len(arr_squared_all) == 0):#if they are void, initialize
        ...
        out_log.writelines("The median mse now is: "+str(numpy.median(arr_squared))+".\n")
        print("The median mse now is:",numpy.median(arr_squared),".")
    else:
    ...
    if(j > 0):
        ...
        print(median_prev,median_this,abs((median_this - median_prev)/median_prev),learning_rate)
        if(median_this < 0.4) & (abs((median_this - median_prev)/median_prev) < 0.001):
            if(flag == 1):
                flag = flag +1
            elif (flag == 2):
                flag = flag +1
    ...
    out_log.writelines("MSE calculation for round "+str(j+1)+" takes "+str(end-start)+"s.\n")
    print("MSE calculation for round ",j+1," takes ", end-start, "s.")
end_train = time.time()
out_log.writelines("Total time for training takes "+str(end-start)+"s.\n")
print("Total time for training takes", end-start, "s.")

if(median_this > 0.4): #set to a big number (such as 10000) to ignore
    out_log.writelines("Warning: internal control of MSE did not pass the filter (median MSE:"+str(median_this)+").\n Imputation can be risky, exit ibNN.\n Modify filter at line 447 to ignore this.\n")
